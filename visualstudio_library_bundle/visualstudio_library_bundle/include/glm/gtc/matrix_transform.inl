...
#		endif
	...
	GLM_FUNC_QUALIFIER tmat4x4<T, defaultp> twoPointPerspectiveRH(T left, T right, T bottom, T top, T nearVal, T farVal) {
		...
		//T sampleSin = static_cast<T>(sin(glm::radians(sampleAngle)));
		//T sampleCos = static_cast<T>(cos(glm::radians(sampleAngle)));
		//T twoTimesNearCosSqr = static_cast<T>(2) * nearVal * sampleCos * sampleCos;
		//T twoTimesNearSinSqr = static_cast<T>(2)* nearVal* sampleSin* sampleSin;
		//T twoTimesNearCosSin = static_cast<T>(2) * nearVal * sampleCos * sampleSin;
		//T cosTimesRightPlusLeft = sampleCos * (right + left);
		//T cosTimesMinusFarMinusNear = sampleCos * (-farVal - nearVal);
		//T sinTimesRightPlusLeft = sampleSin * (right + left);
		//T sinTimesMinusFarMinusNear = sampleSin * (-farVal - nearVal);
		//T cosTimesTopPlusBottom = sampleCos * (top + bottom);
		//T sinTimesTopPlusBottom = sampleSin * (top + bottom);


		//T res00 = twoTimesNearCosSqr / (right - left) + sampleSin * (cosTimesRightPlusLeft / (right - left) + sinTimesMinusFarMinusNear / (farVal - nearVal));
		//T res01 = sinTimesTopPlusBottom / (top - bottom);
		//T res02 = twoTimesNearCosSin / (right - left) + sampleSin * (cosTimesMinusFarMinusNear / (farVal - nearVal) - sinTimesRightPlusLeft / (right - left));
		//T res03 = -sampleSin;
		//T res20 = -twoTimesNearCosSin / (right - left) + sampleCos * (cosTimesRightPlusLeft / (right - left) + sinTimesMinusFarMinusNear / (farVal - nearVal));
		//T res21 = cosTimesTopPlusBottom / (top - bottom);
		//T res22 = twoTimesNearSinSqr / (right - left) + sampleCos * (cosTimesMinusFarMinusNear / (farVal - nearVal) - sinTimesRightPlusLeft / (right - left));
		//T res23 = -sampleCos;
		//T res30 = -(static_cast<T>(2) * farVal * nearVal * sampleSin) / (farVal - nearVal);
		//T res32 = -(static_cast<T>(2) * farVal * nearVal * sampleCos) / (farVal - nearVal);
...
#		endif
		...
		return rotY* Result * rotMinusY;
	}

	template <typename T>
	GLM_FUNC_QUALIFIER tmat4x4<T, defaultp> threePointPerspectiveRH(T left, T right, T bottom, T top, T nearVal, T farVal) {
		T sampleAngle = static_cast<T>(60.0f);
		T sampleAngle2 = static_cast<T>(45.0f);
		T sampleSin = static_cast<T>(sin(glm::radians(sampleAngle)));
		T sampleCos = static_cast<T>(cos(glm::radians(sampleAngle)));
		T sampleSin2 = static_cast<T>(sin(glm::radians(sampleAngle2)));
		T sampleCos2 = static_cast<T>(cos(glm::radians(sampleAngle2)));

		tmat4x4<T, defaultp> Result(0);
		Result[0][0] = (static_cast<T>(2) * nearVal) / (right - left);
		Result[0][2] = -(farVal * nearVal * sampleSin) / (farVal - nearVal);
		Result[1][1] = (static_cast<T>(2) * nearVal) / (top - bottom);
		Result[1][2] = -(farVal * nearVal * sampleCos * sampleSin2) / (farVal - nearVal);
		Result[2][0] = (right + left) / (right - left);
		Result[2][1] = (top + bottom) / (top - bottom);
		Result[2][3] = static_cast<T>(-1);
...
#		if GLM_DEPTH_CLIP_SPACE == GLM_DEPTH_ZERO_TO_ONE
		...
		Result[3][2] = -(farVal * nearVal) / (farVal - nearVal);
#		else
		Result[2][2] = -(farVal * nearVal * sampleCos * sampleCos2) / (farVal - nearVal);
		Result[3][2] = -(static_cast<T>(2) * farVal * nearVal) / (farVal - nearVal);
#		endif
